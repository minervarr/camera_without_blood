...
public:
    ...
    // Absolute path of the <clip>_ai.flac side-car if one was fully written by the
    // last finalize_denoise(); empty otherwise. Used to report the file to a host.
    const std::string& ai_written_path() const { return ai_written_path_; }
...
private:
    ...
    // Returns true only when the file was fully written.
    bool write_flac_file(const std::string& path,
    ...
    std::string            ai_written_path_;        // set to ai_path_ once fully written
